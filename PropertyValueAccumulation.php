 ...
 * Also you can change accumulating order passing 1 (straight) or 0 (reverse) as PropertyValueAccumulation::getAccumulated() method's second argument.
 */
trait PropertyValueAccumulation
{
	 ...
	 * @param integer $order optional accumulating order (0 - from child to parent, 1 - "natural" order, from parent to child).
	 ...
	 */
	public static function getAccumulated($name, $order = 1, array $args = [])
	{
		return $order ? 
		array_merge(self::getListLegacy($name, $order, $args), self::getListIncrement($name, $args)) :
		array_merge(self::getListIncrement($name, $args), self::getListLegacy($name, $order, $args));
	}
	 ...
	 * @return array accumulated value.
	 */
	private static function getListLegacy($name, $order, array $args = [])
	{
		$parent = get_parent_class(get_called_class());
		return ($parent !== false && method_exists($parent, 'getAccumulated')) ? 
			call_user_func_array([$parent, 'getAccumulated'], [$name, $order, $args]) :
			[];
	}
